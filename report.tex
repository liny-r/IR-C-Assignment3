...
\textbf{Reading the stratifications.} Vintage is the dominant driver because it proxies for the origination-rate environment and the subsequent opportunity to refinance. FICO and LTV show cleaner but smaller credit-quality gradients: higher-credit and moderate-LTV borrowers are more refinance-capable, while very high-LTV loans prepay more slowly. Loan purpose is not purely monotone: no-cash-out refinances have the highest overall prepayment incidence, but purchase loans fall earlier in the curve, consistent with borrower mobility and sale-driven payoff timing.
...
\textbf{Setup.} Prepayment and default are competing risks: each loan exits via exactly one cause, and treating one as random censoring distorts the other. We compare the cause-specific Kaplan--Meier estimator used in Parts A--D against the Aalen--Johansen (AJ) estimator, which keeps all exits in the risk-set denominator and provides a coherent probability partition.

\textbf{KM bias.} KM censors competing-event loans, shrinking the denominator at each event time and inflating the cause-specific incidence:
\[
...
\textbf{Stratified competing risks.} Stratifying the AJ CIF by FICO, LTV, and vintage reveals an asymmetric pattern: FICO and LTV strongly separate \emph{default} curves (low-FICO borrowers default at $6\times$ the rate of high-FICO), but leave the \emph{prepayment} curves nearly indistinguishable. Vintage dominates both. The interpretation is that credit quality determines who \emph{can} refinance (default risk), not who \emph{wants} to (refinance behaviour), while macro rate environment at origination determines both.

\textbf{Cause-specific Cox.} We extend the Cox framework to fit separate proportional-hazards models for prepayment and default:
\[
  h_k(t \mid \mathbf{x}) = h_{0k}(t)\,\exp\!\bigl(\boldsymbol{\beta}_k^\top\mathbf{x}\bigr),
  \qquad k \in \{\mathrm{pre},\,\mathrm{def}\},
\]
where the cause-$k$ model treats all competing-cause exits as censored. Beyond the Part B origination features, we add two delinquency variables: \texttt{max\_delinquency} (months past due over loan life) and \texttt{ever\_90+DQ} (binary severe-delinquency flag).
...
\textbf{Key takeaways E(i).} (i) KM systematically overstates prepayment incidence; use AJ for any application requiring coherent probabilities across competing causes. (ii) Vintage era dominates FICO and LTV for prepayment; FICO and delinquency dominate for default. (iii) A combined Cox model gives the wrong sign for credit-risk variables --- cause-specific (or Fine--Gray) is mandatory whenever default is a non-trivial outcome.

% -----------------------------------------------------------------
\subsection{Part E(ii) --- Time-Varying Covariates: Andersen--Gill}

\textbf{Motivation.} The Cox models in Parts B, D, and E(i) all use origination-time covariates only. In reality, the variables that drive prepayment evolve month by month: the refinancing incentive depends on \emph{today's} market rate, not the rate at origination; equity builds via amortisation and HPI; macro variables (unemployment, HPI growth) move through cycles. The Andersen--Gill (AG) Cox extension addresses this by fitting one row per loan-month:
\[
  h_i(t) = h_0(t) \cdot \exp\!\bigl(\boldsymbol{\beta}^\top \mathbf{x}_i(t)\bigr),
\]
where $\mathbf{x}_i(t)$ is the borrower's covariate vector \emph{at month $t$}. The partial likelihood and Breslow baseline estimator are formally identical to standard Cox; only the data layout changes.

\textbf{Sample.} 100K loans drawn from the 2M-loan panel with monthly performance data; $\sim$47 months average duration yields 4.68M loan-month intervals. The static baseline (``Standard Cox'') uses the same 100K loans with covariates fixed at origination.
...
\textbf{Key takeaways E(ii).} (i) Monthly-updated covariates add real information --- log-likelihood gain $+1{,}996$, $\Delta C = +0.07$ at $T=12$. (ii) Rate incentive is the dominant driver; macro coefficients require careful inspection because vintage-era and current-month signals can have opposite interpretations. (iii) TV Cox degrades beyond 70 months due to survivorship bias; the principled fix is landmarking.

% -----------------------------------------------------------------
\subsection{Part E(iii) --- Neural Survival: LongitudinalDeepHit}

\textbf{Motivation.} Cox and Andersen--Gill assume that covariate effects are constant over time, captured by a single coefficient vector. The Schoenfeld test in Part B already rejects PH for 12/15 covariates, and the long-horizon C-index reversal in E(ii) shows that the same monthly covariate has different predictive weight at different points in the loan's life. A natural next step is a sequence model that reads the full monthly history $[\mathbf{x}_1, \dots, \mathbf{x}_T]$ and produces nonlinear, time-varying hazards for prepayment and default jointly.

\textbf{Architecture.} We adapt the DeepHit framework (Lee et al.\ 2018) to longitudinal sequences via a Transformer encoder:
\begin{itemize}[leftmargin=1.2em,itemsep=2pt,topsep=2pt]
...
\textbf{Key takeaways E(iii).} (i) Default AUC matches Part D's DeepCox benchmark with $6\times$ less training data, validating the sequence-model design. (ii) Feature importance is time-varying and can be visualised over the loan lifecycle, uniquely among the models in this study. (iii) The known limitation is calibration: the 1.6\% event rate causes NLL collapse and unreliable absolute probabilities. AUC discrimination is preserved by the ranking loss but absolute hazards are not trustworthy; focal-loss upweighting is the priority remediation.

% -----------------------------------------------------------------
\subsection{Part E(iv) --- Scenario Analysis: MBS Prepayment under Rate Shocks}

\textbf{Motivation.} The ultimate use of a prepayment model in industry is to stress-test MBS portfolios under interest-rate scenarios: how does prepayment speed change when rates move $\pm$100, $\pm$200, $\pm$300\,bp? How does that translate into Weighted Average Life (WAL) and price? Static models such as Part D's DeepCox have no rate channel --- the origination coupon is fixed --- so the only way to shock them is to retrain on shifted rates, which is operationally infeasible and conceptually wrong. The TV Cox model from E(ii) provides the cleanest direct rate channel: \texttt{rate\_incentive} is a live covariate, so any market-rate shock $\Delta r$ maps directly to a log-hazard shift via the estimated coefficient.

\textbf{TV Cox rate channel.} The closed-form response is
\[
...
\end{itemize}
The asymmetry is small in this specification ($\$0.05$) because TV Cox has a linear rate channel and the baseline market rate of 5.3\% is close to the par-coupon level. In practice, real MBS exhibit much larger convexity asymmetry because (i) prepayment hazard is nonlinear (S-curve) in rate incentive at the population level, (ii) heterogeneity across borrowers amplifies the rate-down response, and (iii) the call-option-like feature of prepayment creates path-dependent valuation. Capturing the full S-curve would require either a nonlinear rate response (focal-loss-fixed DeepHit) or a population mixture model.

\textbf{Key takeaways E(iv).} (i) Static models have no usable rate channel; time-varying covariates are required for any scenario analysis worth doing. (ii) TV Cox's linear rate channel makes stress-test results fully explainable from one coefficient, valuable for auditability. (iii) Negative convexity is correctly captured directionally; magnitude is small in this single-loan specification and would grow in a heterogeneous pool. (iv) FICO and LTV effects on the rate response are real but require more loans to surface statistically --- in this specification rate incentive drives all the CPR variation.
...
\textbf{Limitations of Parts A--D (addressed in Part E).}
\begin{enumerate}[leftmargin=1.5em,itemsep=2pt,topsep=2pt]
  \item \emph{Time-varying covariates.} Parts B and D use only origination-time features. \textbf{Addressed in E(ii)} via the Andersen--Gill loan-month panel; the four time-varying features (\texttt{rate\_incentive}, ELTV, unemployment, hpi\_yoy) improve landmark C-index by $+0.07$ at $T = 12$.
  \item \emph{Competing risks.} Parts A--D censor default as a competing exit. \textbf{Addressed in E(i)} via Aalen--Johansen and cause-specific Cox; the naive combined Cox model gives the wrong sign for credit-risk variables.
  \item \emph{Scenario analysis.} The trained Deep Cox is not directly stressable because rate is fixed at origination. \textbf{Addressed in E(iv)} using the TV Cox model from E(ii), translating rate shocks into MBS WAL and price under negative convexity.
\end{enumerate}
...
\begin{enumerate}[leftmargin=1.5em,itemsep=2pt,topsep=2pt]
  ...
  \item The LongitudinalDeepHit model in E(iii) suffers NLL collapse due to the 1.6\% event rate; AUC discrimination is preserved but absolute hazards are unreliable. Focal-loss reweighting is the priority remediation.
  \item TV Cox degrades beyond 70 months due to survivorship bias in the rate-locked population; a landmark super-model would fix this.
\end{enumerate}
